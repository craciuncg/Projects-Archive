...
void Lab5::Init()
{
    ...
    for (int i = 0; i < 40; ++i)
    {
        ...
        lightInfo.position = glm::vec3(randf(-10.0f, 10.0f), randf(0.0f, 3.0f), randf(-10.0f, 10.0f));
        lightInfo.color = glm::vec3(randf(), randf(), randf());
        lightInfo.radius = randf(3.0f, 4.0f);
...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    for (auto& l : lights)
    {
        ...
        float rotationRadians = AI_MATH_PI_F / 30.0f * deltaTimeSeconds;
    ...
    // Lighting pass
    {
        ...
        for (auto& lightInfo : lights)
        {
            ...
            shader->Use();
            glUniform3f(glGetUniformLocation(shader->program, "light_position"), lightInfo.position.x, lightInfo.position.y, lightInfo.position.z);
            glUniform3f(glGetUniformLocation(shader->program, "light_color"), lightInfo.color.x, lightInfo.color.y, lightInfo.color.z);
            ...
            RenderMesh(meshes["sphere"], shader, lightInfo.position, 2.0f * lightInfo.radius * glm::vec3(1.0f));
...
void Lab5::OnKeyPress(int key, int mods)
{
    ...
    if (key == GLFW_KEY_R) for (auto& lightInfo : lights) {
        lightInfo.position = glm::vec3(randf(-10, 10), randf(0, 3), randf(-10, 10));
        lightInfo.color = glm::vec3(randf(), randf(), randf());
        lightInfo.radius = randf(3.0f, 4.0f);
    }
    if (key == GLFW_KEY_F) for (int i = 0; i < lights.size(); i++) {
        ...
        while (true) {
            ...
            bool can_break = true;
            for (int j = 0; j < i; j++)
                if (glm::length(lightInfo.position - lights[j].position) * 1.0f < lights[j].radius) {
                    can_break = false;
                    break;
                }
            if (can_break)
